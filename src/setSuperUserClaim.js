...
async function main() {
  ...
  console.log("\n--- Add 'isSuperUser' Claim to User ---"); // タイトルを変更

  const answers = await prompt([
    {
      type: "input",
      name: "email",
      message: "Enter the email address of the user to modify:", // メッセージを少し変更
      validate: (value) => {
    ...
    },
    {
      ...
      name: "confirm",
      message: (answers) =>
        `Add the 'isSuperUser: true' claim to user ${answers.email}?`, // 確認メッセージを更新
      default: false,
    },
  ]);
  ...
  try {
    ...
    // 3. 既存のクレームに isSuperUser: true を追加（または上書き）
    const updatedClaims = {
      ...existingClaims,
      isSuperUser: true, // ★★★ ここを isSuperUser: true に変更 ★★★
    };
    ...
    console.log(
      `\n✅ Successfully set 'isSuperUser: true' claim for user ${email} (UID: ${uid}).` // 成功メッセージを更新
    );
