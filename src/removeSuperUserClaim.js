...
async function main() {
  ...
  console.log("\n--- Set 'isSuperUser' Claim to false for User ---"); // タイトルを変更

  const answers = await prompt([
    {
      type: "input",
      name: "email",
      message: "Enter the email address of the user to modify:",
      validate: (value) => {
    ...
    },
    {
      ...
      name: "confirm",
      message: (answers) =>
        `Set the 'isSuperUser: false' claim for user ${answers.email}? (This will effectively remove superuser privileges if the claim exists)`, // 確認メッセージを更新
      default: false,
    },
  ]);
  ...
  try {
    ...
    // 3. 既存のクレームに isSuperUser: false を追加（または上書き）
    const updatedClaims = {
      ...existingClaims,
      isSuperUser: false, // ★★★ ここを isSuperUser: false に変更 ★★★
    };
    ...
    console.log(
      `\n✅ Successfully set 'isSuperUser: false' claim for user ${email} (UID: ${uid}).` // 成功メッセージを更新
    );
